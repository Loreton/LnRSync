...
# #
# ##############################################################
def lastRunLog(data='', fName=None, quit=False):
    ...
    if (fName):
        ...
        if not os.path.isfile(fName):
            ...
            msg = 'Il file {} esiste. Vuoi rimpiazzarlo? [y/N]'.format(fName)
...
def Main(gv, action):
    ...
    if action == 'action01':
        choice = gv.Ln.getKeyboardInput(gv, "    uscita Temporanea... sviluppare codice" , keySep=",", validKeys='', exitKey='ENTER', deepLevel=1)
        '''
        SAMPLE per leggere il file ini
        iniFileName = os.path.abspath(os.path.join(gv.Prj.configDIR, gv.Prj.name + '_config.ini'))
        iniConfigParser, iniDict = gv.Ln.readIniFile(iniFileName, RAW=False, exitOnError=True)

        gv.iniDict = gv.Ln.LnDict(iniDict)
        gv.iniDict.printDict(gv)
        '''
...
def Mai2n(gv, args):
    ...
    DRY_RUN = '--dry-run'
    for inx, item in enumerate(args):
        if item.upper() in ['--GO']:
            DRY_RUN = ''
            del args[inx]
    ...
    if len(args) >= 2:
        iniFileName = os.path.abspath(os.path.join(gv.scriptDir, args[1]))
    ...
    # lastRunLog(fName='d:/tmp/LAST_LOG.txt')
    lastRunLog(fName=os.path.abspath(os.path.join(gv.scriptDir, '../', 'conf/LAST_RUN.log')))
    ...
    for key, val in workingSECT.items():
        ...
        for subDir in subDirs:
            ...
            currentPaths = "{0:<70} - {1}".format(gv.sourcePATH, gv.destPATH)
            # for item in lastRunData: print (item)
            if currentPaths in lastRunData:
                print ("skipping --> {0}".format(currentPaths))
                continue
            else:
                print ("- going to copy --> {0}".format(currentPaths))
