...
if __name__ == "__main__":
    ...
    cmd = 'netstat -an'
   # retVal = LN.proc.runCommand(gv, cmd, argsList=[], exit=False)
    # retVal = LN.proc.runProcess(gv, cmd, argsList=[], PWAIT=True, timeOut=None, stdOUTfile=False)
#    retVal = LN.proc.runCommand_New_Vedi_LnRSync(gv, cmd, wkdir='.', argsList=[], exit=False)
    retVal = LN.proc.runCommand_New(gv, cmd, wkdir='.', argsList=[], PWAIT=99, stdOUTfile=False, exit=False)
